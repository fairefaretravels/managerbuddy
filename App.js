...
const s = {
  ...
  statVal: { fontFamily: "'Georgia', serif", fontSize: 28, fontWeight: 500, color: (accent) => accent ? "#C4622D" : "#1C1C1A", lineHeight: 1 },
  ...
  badge: (type) => {
    const map = { green: { background: "#EAF3DE", color: "#3B6D11" }, amber: { background: "#FAEEDA", color: "#854F0B" }, red: { background: "#FCEBEB", color: "#A32D2D" }, blue: { background: "#E6F1FB", color: "#185FA5" }, gray: { background: "#F1EFE8", color: "#5F5E5A" } };
    return { ...map[type], display: "inline-flex", alignItems: "center", padding: "3px 9px", borderRadius: 20, fontSize: 11, fontWeight: 500 };
  },
  ...
  alert: (type) => {
    const map = { orange: { background: "#FAF0EA", border: "0.5px solid #E8A882", color: "#C4622D" }, green: { background: "#EAF3DE", border: "0.5px solid #C0DD97", color: "#3B6D11" }, blue: { background: "#E6F1FB", border: "0.5px solid #B5D4F4", color: "#185FA5" } };
    return { ...map[type], display: "flex", alignItems: "center", gap: 10, padding: "8px 12px", borderRadius: 6, fontSize: 13 };
  },
};
...
function StatCard({ label, val, sub, accent }) {
  return (
    <div style={s.stat(accent)}>
      ...
      <div style={{ ...s.statVal(accent), fontFamily: "'Georgia', serif", fontSize: 28, fontWeight: 500, lineHeight: 1 }}>{val}</div>
...
function FormInput({ id, placeholder, type, value, onChange, children }) {
  if (children) return <select id={id} style={s.formInput} value={value} onChange={onChange}>{children}</select>;
  return <input id={id} style={s.formInput} placeholder={placeholder} type={type || "text"} value={value} onChange={onChange} />;
}
...
function Overview({ reservations, switchTab }) {
  ...
  function cycleTable(i) { setTableStates(ts => ts.map((t, idx) => idx === i ? TABLE_CYCLE[(TABLE_CYCLE.indexOf(t) + 1) % 3] : t)); }
...
function Reservations({ reservations, setReservations }) {
  ...
  function toggle(i) { setReservations(rs => rs.map((r, idx) => idx === i ? { ...r, status: r.status === "confirmed" ? "pending" : "confirmed" } : r)); }
  function add() {
    ...
    setReservations(rs => [...rs, { time: `${h12}:${m} ${ampm}`, guest: form.guest, size: parseInt(form.size) || 2, table: form.table, notes: form.notes, status: "confirmed" }]);
    ...
    <div>
      ...
      <Card>
        <div style={{ ...s.cardBody, padding: 0, overflowX: "auto" }}>
          <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13 }}>
            <thead>
              <tr>{["Time","Guest","Party","Table","Notes","Status",""].map(h => <th key={h} style={{ textAlign: "left", padding: "8px 12px", fontSize: 11, fontWeight: 500, color: "#6B6B67", letterSpacing: "0.04em", textTransform: "uppercase", borderBottom: "0.5px solid #E8E8E3" }}>{h}</th>)}</tr>
            </thead>
            <tbody>
              {filtered.map((r, i) => (
                <tr key={i} style={{ cursor: "pointer" }}>
                  ...
                  <td style={{ padding: "10px 12px", borderBottom: "0.5px solid #E8E8E3" }}><Btn onClick={() => toggle(reservations.indexOf(r))} style={{ padding: "3px 8px", fontSize: 11 }}>Toggle</Btn></td>
                </tr>
              ))}
            </tbody>
          </table>
      ...
      <div style={{ ...s.twoCol, marginTop: 12 }}>
        <Card>
          ...
          <div style={s.cardBody}>
            <div style={s.formRow}>
              <div style={s.formGroup}><label style={s.formLabel}>Guest name</label><input style={s.formInput} placeholder="Last name" value={form.guest} onChange={e => setForm(f => ({ ...f, guest: e.target.value }))} /></div>
              <div style={s.formGroup}><label style={s.formLabel}>Party size</label><input style={s.formInput} type="number" min="1" max="20" value={form.size} onChange={e => setForm(f => ({ ...f, size: e.target.value }))} /></div>
            </div>
            <div style={s.formRow}>
              <div style={s.formGroup}><label style={s.formLabel}>Time</label><input style={s.formInput} type="time" value={form.time} onChange={e => setForm(f => ({ ...f, time: e.target.value }))} /></div>
              <div style={s.formGroup}><label style={s.formLabel}>Table</label>
            ...
            <div style={s.formGroup}><label style={s.formLabel}>Notes</label><input style={s.formInput} placeholder="Anniversary, allergies, special requests..." value={form.notes} onChange={e => setForm(f => ({ ...f, notes: e.target.value }))} /></div>
            ...
            {[["Dining room", 90, "#C4622D"], ["Bar seating", 50, "#1C1C1A"], ["Private room", 60, "#1C1C1A"]].map(([label, pct, col]) => (
              <div key={label} style={{ marginBottom: 14 }}>
                <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 6 }}>
                  ...
                  <span style={{ fontSize: 12, fontWeight: 500 }}>{Math.round(pct * 0.9)} / {Math.round(pct * 0.9 / pct * 100)}%</span>
                </div>
                <div style={{ height: 6, background: "#E8E8E3", borderRadius: 3 }}><div style={{ width: `${pct}%`, height: "100%", background: col, borderRadius: 3 }} /></div>
              </div>
            ))}
    ...
    <div>
      ...
      <div style={s.twoCol}>
        <Card>
          ...
          <div style={s.cardBody}>
            {staff.map((st, i) => {
              const span = Math.min(Math.abs(timeToNum(st.end) - timeToNum(st.start)), 10);
        ...
        <Card>
          ...
          <div style={s.cardBody}>
            <div style={s.formGroup}><label style={s.formLabel}>Staff member</label><input style={s.formInput} placeholder="Name" value={form.name} onChange={e => setForm(f => ({ ...f, name: e.target.value }))} /></div>
            ...
            <div style={s.formRow}>
              <div style={s.formGroup}><label style={s.formLabel}>Start</label><input style={s.formInput} type="time" value={form.start} onChange={e => setForm(f => ({ ...f, start: e.target.value }))} /></div>
              <div style={s.formGroup}><label style={s.formLabel}>End</label><input style={s.formInput} type="time" value={form.end} onChange={e => setForm(f => ({ ...f, end: e.target.value }))} /></div>
            </div>
            <div style={s.formGroup}><label style={s.formLabel}>Status</label>
              <select style={s.formInput} value={form.status} onChange={e => setForm(f => ({ ...f, status: e.target.value }))}>
                <option value="confirmed">Confirmed</option><option value="pending">Pending</option><option value="open">Open — needs fill</option>
              </select>
            </div>
...
function Menu({ menu, setMenu }) {
  ...
  function adjust(i, d) { setMenu(ms => ms.map((m, idx) => idx === i ? { ...m, stock: Math.max(0, m.stock + d) } : m)); }
  function add() {
    if (!form.name.trim()) return;
    ...
    <div>
      ...
      <div style={{ display: "flex", gap: 8, marginBottom: 16 }}>
        ...
        <select style={{ ...s.formInput, width: "auto", fontSize: 12 }} value={stockFilter} onChange={e => setStockFilter(e.target.value)}>
          <option value="all">All stock</option><option value="low">Low stock</option><option value="ok">In stock</option>
        </select>
      </div>
          ...
          return (
            <div key={i} style={{ background: "#fff", border: "0.5px solid #E8E8E3", borderRadius: 8, padding: "12px 14px" }}>
              ...
              <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginTop: 10 }}>
                <Badge type={low ? "red" : "green"}>{m.cat === "Cocktail" ? "In stock" : low ? `Low — ${m.stock} left` : `${m.stock} portions`}</Badge>
      ...
      <Card style={{ marginTop: 16 }}>
        ...
        <div style={s.cardBody}>
          <div style={s.formRow}>
            <div style={s.formGroup}><label style={s.formLabel}>Item name</label><input style={s.formInput} placeholder="Dish or drink name" value={form.name} onChange={e => setForm(f => ({ ...f, name: e.target.value }))} /></div>
          ...
          <div style={s.formRow}>
            <div style={s.formGroup}><label style={s.formLabel}>Price</label><input style={s.formInput} type="number" placeholder="28" value={form.price} onChange={e => setForm(f => ({ ...f, price: e.target.value }))} /></div>
            <div style={s.formGroup}><label style={s.formLabel}>Stock count</label><input style={s.formInput} type="number" placeholder="20" value={form.stock} onChange={e => setForm(f => ({ ...f, stock: e.target.value }))} /></div>
          </div>
...
function Social({ posts, setPosts }) {
  ...
  function add() {
    ...
    setPosts(ps => [{ platform: form.platform, text: form.text, tags: form.tags, date: form.date && form.time ? `${form.date} ${form.time}` : "Scheduled", status: "scheduled" }, ...ps]);
    ...
    <div>
      ...
      <div style={s.twoCol}>
        ...
        <Card>
          ...
          <div style={s.cardBody}>
            <div style={s.formGroup}><label style={s.formLabel}>Platform</label>
            ...
            <div style={s.formGroup}><label style={s.formLabel}>Caption</label><textarea style={{ ...s.formInput, resize: "vertical" }} rows={3} placeholder="Write your caption..." value={form.text} onChange={e => setForm(f => ({ ...f, text: e.target.value }))} /></div>
            <div style={s.formGroup}><label style={s.formLabel}>Hashtags</label><input style={s.formInput} placeholder="#finedining #cocktails" value={form.tags} onChange={e => setForm(f => ({ ...f, tags: e.target.value }))} /></div>
            <div style={s.formRow}>
              <div style={s.formGroup}><label style={s.formLabel}>Date</label><input style={s.formInput} type="date" value={form.date} onChange={e => setForm(f => ({ ...f, date: e.target.value }))} /></div>
              <div style={s.formGroup}><label style={s.formLabel}>Time</label><input style={s.formInput} type="time" value={form.time} onChange={e => setForm(f => ({ ...f, time: e.target.value }))} /></div>
            </div>
...
const TAB_LABELS = { overview: "Overview", reservations: "Reservations", staff: "Staff & Shifts", menu: "Menu & Inventory", social: "Social & Marketing" };
